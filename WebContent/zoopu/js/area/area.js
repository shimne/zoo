var _area_data = 
{
	...
	c: {'北京市': ['朝阳区', '海淀区', '西城区', '东城区', '崇文区', '宣武区', '丰台区', '石景山区', '门头沟', '房山区', '通州区', '大兴区', '顺义区', '怀柔区', '密云区', '昌平区', '平谷区', '延庆县'],
		'天津市': ['东丽区', '和平区', '河北区', '河东区', '河西区', '红桥区', '南开区', '塘沽区', '西青区', '武清区', '津南区', '汉沽区', '大港区', '北辰区', '宝坻区', '宁河县', '静海县', '蓟县'],
		...
		'上海市': ['徐汇区', '长宁区', '静安区', '闸北区', '虹口区', '杨浦区', '宝山区', '闵行区', '嘉定区', '浦东新区', '青浦区', '松江区', '金山区', '奉贤区', '普陀区', '黄浦区', '崇明县'],
		...
		'重庆市': ['万州区', '涪陵区', '南川区', '大足区', '黔江区', '合川区', '巴南区', '北碚区', '江津区', '渝北区', '长寿区', '永川区', '江北区', '南岸区', '九龙坡区', '沙坪坝区', '大渡口区', '綦江区', '渝中区', '高新区', '北部新区', '梁平县', '潼南县', '武隆县', '丰都县', '奉节县', '开县', '云阳县', '忠县', '巫溪县', '巫山县', '石柱县', '彭水县', '垫江县', '酉阳县', '秀山县', '璧山县', '荣昌县', '铜梁县', '城口县'],
...
{
	var options = $.fn.extend({ids: []}, options);
	var areaDatas = _area_data;

	var pInputId = options.ids[0];
	var cInputId = options.ids[1];
	var tInputId = options.ids[2];

	var pInput = $('#' + pInputId);
	var cInput = $('#' + cInputId);
	var tInput = $('#' + tInputId);

	var pList = getProvinceList();
	var cList = [];
	var tList = [];

	var pValue = pInput.val() ? pInput.val() : null;
	var cValue = cInput.val() ? cInput.val() : null;
	var tValue = tInput.val() ? tInput.val() : null;

	var pCss;
	var cCss;
	var tCss;

	var pDiv = $('<div id="_' + pInputId + 'Div" class="countys clearfix" style="display:none;"></div>').appendTo('body');
	var cDiv = $('<div id="_' + cInputId + 'Div" class="countys clearfix" style="display:none;"></div>').appendTo('body');
	var tDiv = $('<div id="_' + tInputId + 'Div" class="countys clearfix" style="display:none;"></div>').appendTo('body');
	...
	this.init = function ()
	{
        ...
        $(document).delegate('html', 'click', function (event) {
            ...
            if (target.closest(pInput).length == 0 && target.closest(cInput).length == 0 && target.closest(tInput).length == 0) {
                divsHide();
            }
        });
    }
    ...
    function showProvinceDiv()
    {
    	...
    	pCss = pCss || {
            'top': pInput.offset().top + pInput.outerHeight() + 'px',
            'left': pInput.offset().left + 'px'
        };

        createP().css(pCss).show();
    }
    ...
    function showCityDiv()
    {
        ...
        cList = getCityList(pValue);

        cCss = cCss || {
            'top': cInput.offset().top + cInput.outerHeight() + 'px',
            'left': cInput.offset().left + 'px'
        };

        createC(cList).css(cCss).show();
    }

    function showTownDiv()
    {
    	divsHide();

        if (!pValue || pValue.length == 0 || !cValue || cValue.length == 0)
        {
        	return;
        }

        tList = getTownList(pValue, cValue);

        tCss = tCss || {
            'top': tInput.offset().top + tInput.outerHeight() + 'px',
            'left': tInput.offset().left + 'px'
        };

        createT(tList).css(tCss).show();
    }
    ...
    function getCityList(pv)
    {
        return areaDatas['c'][pv];
    }

    function getTownList(pv, cv)
    {
        return areaDatas['a'][pv + '-' + cv];
    }
    ...
    function createP()
    {
        ...
        {
            html += '<a class="a" href="javascript:void(0);">' + pList[i] + '</a>';
        }
    ...
    function createC(citys)
    {
        ...
        {
        	html += '<a class="a" href="javascript:void(0);">' + citys[i] + '</a>';
        }
    ...
    function createT(towns)
    {
        var html = '';

        for (var i = 0; i < towns.length; i++)
        {
        	html += '<a class="a" href="javascript:void(0);">' + towns[i] + '</a>';
        }

        tDiv.html(html);

        tDiv.delegate('a', 'click', function (event) {
            var target = $(this);
            tValue = target.text();
            writeT();
            divsHide();
